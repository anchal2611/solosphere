...
export default function Layout() {
  const { notifications, profile } = useContext(AppContext);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-brand-bg-warm selection:bg-brand-gold/30 selection:text-brand-text-dark relative">
      ...
      <header className="flex md:hidden items-center justify-between px-6 py-4 bg-brand-bg-beige border-b border-brand-text-muted/10 sticky top-0 z-40 shadow-sm animate-fade-in">
        ...
        <div className="fixed inset-0 bg-brand-text-dark/40 z-50 transition-opacity duration-300 md:hidden">
          <div className="absolute top-0 left-0 bottom-0 w-4/5 max-w-xs bg-brand-bg-beige p-6 shadow-2xl flex flex-col animate-slide-right">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-2" onClick={() => { navigate('/dashboard'); setMobileMenuOpen(false); }}>
                <Compass className="text-brand-terracotta cursor-pointer" size={20} />
                <span className="font-serif font-bold text-lg text-brand-text-dark cursor-pointer">SoloSphere</span>
              </div>
              <button 
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3.5 px-4 py-2.5 rounded-brand text-left font-medium transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] ${
                      isActive 
            ...
            <button
              ...
              className="w-full flex items-center gap-3.5 px-4 py-2.5 rounded-brand text-left font-medium text-brand-text-muted hover:bg-brand-bg-warm hover:text-brand-terracotta transition-colors mt-2"
            >
      ...
      <aside className="hidden md:flex flex-col w-20 lg:w-64 bg-brand-bg-beige border-r border-brand-text-muted/10 h-screen sticky top-0 self-start px-4 py-8 z-30 transition-all duration-300 shadow-sm justify-between shrink-0 animate-fade-in">
        
        <div className="flex flex-col">
          {/* Logo / Header */}
          <div 
            className="flex items-center gap-3 px-3 mb-8 justify-center lg:justify-start cursor-pointer"
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => handleTabClick(item.id)}
                  className={`w-full relative group flex items-center gap-3.5 px-4 py-2.5 rounded-brand transition-all duration-300 justify-center lg:justify-start hover:scale-[1.02] active:scale-[0.98] ${
                    isActive 
                      ? 'bg-brand-terracotta text-brand-bg-warm shadow-md' 
                      : 'text-brand-text-muted hover:bg-brand-bg-warm hover:text-brand-text-dark'
                  }`}
        ...
        {/* Desktop Profile Info & Bottom Greeting Card */}
        <div className="flex flex-col gap-4 border-t border-brand-text-muted/10 pt-6">
          
          {/* Card-backed Greeting Widget */}
          <div className="hidden lg:flex items-center gap-2.5 bg-white/40 border border-brand-text-muted/5 rounded-brand p-3 shadow-xs select-none">
            <GreetingIcon size={15} className="text-brand-gold shrink-0 animate-pulse" />
            <div className="flex flex-col text-left">
              <span className="font-sans text-[10px] font-bold text-brand-text-dark leading-tight">{timeGreeting}</span>
              <span className="font-sans text-[9px] text-brand-text-muted leading-tight mt-0.5">Companion Mode</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 px-2 justify-center lg:justify-start">
            ...
            <div className="hidden lg:block">
              ...
              <p className="font-sans text-xs text-brand-text-muted mt-0.5">My quiet space</p>
            </div>
          </div>

          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3.5 px-4 py-2.5 rounded-brand text-brand-text-muted hover:bg-brand-bg-warm hover:text-brand-terracotta transition-colors justify-center lg:justify-start font-sans text-xs font-semibold hover:scale-[1.02] active:scale-[0.98]"
            title="Exit Space"
          >
